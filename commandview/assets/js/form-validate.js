	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#tkids-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#tkids-contact').find('input,textarea').attr({value:''});
				jQuery('#tkids-contact').css({opacity:1});
				jQuery('#tkids-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#tkids-contact').animate({opacity:0.3});
		jQuery('#tkids-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#tkids-contact').find('input,textarea').css('border','none');
