...
$(function() {
    ...
    if (id_msg != "") {
        var dados = {
            btn_listar_Contato  : true,
            id_msg              : id_msg
        }
        $.get('api/controllers/Contato.php', dados, function(retorna) {
            var Contato = jQuery.parseJSON(retorna);

            $("#tbody_data").html(``);
            if (Contato.type == "success") {
                Contato.data.forEach((dados_encomenda, index) => {
                    var status = `
                ...
                $("#tbody_data").append(`
                    <tr>
                        <td colspan="6" class="text-center">${Contato.data}</td>
                    </tr>
                `);
            }
        });
    ...
    $(document).on('click', '#btn_view_order', function() {
        ...
        $.get('api/controllers/Contato.php', dados, function(retorno) {
